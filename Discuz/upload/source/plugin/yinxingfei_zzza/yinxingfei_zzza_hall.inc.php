...
if(!defined('IN_DISCUZ')) {
	...
	//连续摇奖	
       ...
       if($fybrdays != $zzza_fyb && $fybrdays != '-1'){//检查后台设置是否对比前一次设置更改过
			...
			$queryaafyb = DB::query("SELECT * FROM ".DB::table('yinxingfei_zzza_tj')." WHERE data > '$zzza_fybday'");
            
            while($zzza_fyb_i = DB::fetch($queryaafyb)){
				
				$zzza_fyb_ilasttime = DB::result_first("SELECT zzza_lasttime_u FROM ".DB::table('yinxingfei_zzza_rank')." WHERE zzza_uid = '$zzza_fyb_i[uid]'");             
